...
Generated (UTC): 2025-09-18T16-28-37Z
...
//! JSON-ручки с HMAC+nonce (deposit_json/redeem_json) для периметра (mTLS + Nginx rewrites).

use axum::{
    body,                                   // axum::body::to_bytes(body, limit)
...
use crate::auth::{require_bridge_key, verify_hmac_and_nonce};

const BODY_LIMIT_BYTES: usize = 1_048_576; // 1 MiB — безопасный лимит для JSON тела
...
/// DEPOSIT (strict idempotent по ext_txid)
...
/// REDEEM (идемпотентность по ext_txid: если уже Redeemed — NO-OP; payout async)
...
/* =======================================================================
   PROTECTED JSON ENDPOINTS (mTLS+HMAC perimiter → HMAC+nonce в приложении)
   =======================================================================*/

async fn parse_json<T: for<'de> serde::Deserialize<'de>>(bytes:&[u8]) -> Result<T, String>{
...
pub async fn deposit_json(
    ...
    let body = body::to_bytes(req.into_body(), BODY_LIMIT_BYTES).await.unwrap_or_default();
    if let Err(e)=require_bridge_key(&headers){ return (StatusCode::UNAUTHORIZED, format!("{{\"error\":\"{e}\"}}")); }
    if let Err(e)=verify_hmac_and_nonce(&headers, &body, st.sled()){ return (StatusCode::UNAUTHORIZED, format!("{{\"error\":\"{e}\"}}")); }
    let Ok(payload) = parse_json::<DepositReq>(&body).await else { return (StatusCode::BAD_REQUEST, "{\"error\":\"bad json\"}".into()) };
    deposit(State(st), Json(payload)).await
}
...
pub async fn redeem_json(
    ...
    if let Err(e)=require_bridge_key(&headers){ return (StatusCode::UNAUTHORIZED, format!("{{\"error\":\"{e}\"}}")); }
    if let Err(e)=verify_hmac_and_nonce(&headers, &body, st.sled()){ return (StatusCode::UNAUTHORIZED, format!("{{\"error\":\"{e}\"}}")); }
    let Ok(payload) = parse_json::<RedeemReq>(&body).await else { return (StatusCode::BAD_REQUEST, "{\"error\":\"bad json\"}".into()) };
    redeem(State(st), Json(payload)).await
}
